...
export const ActionPanel: React.FC<ActionPanelProps> = ({
  ...
  return (
    <div className="w-full flex flex-col gap-5">
      ...
      {hasActiveQuestion && (
        <div className="glass-panel-elevated rounded-3xl p-5 sm:p-6 border border-emerald-400/40 shadow-[0_0_35px_rgba(16,185,129,0.25)] relative overflow-hidden specular-border">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-start gap-3.5">
              <div className="w-11 h-11 rounded-2xl bg-gradient-to-tr from-emerald-500/25 to-lime-500/25 border border-emerald-400/50 flex items-center justify-center text-emerald-300 shrink-0 mt-0.5 shadow-[0_0_18px_rgba(16,185,129,0.35)]">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {isQuestionAuthor && (
                    <span className="text-[10px] bg-[#061e14] text-emerald-300 px-2.5 py-0.5 rounded-lg border border-emerald-400/40 font-mono flex items-center gap-1 shadow-inner">
            ...
            {(isQuestionAuthor || isHost) && (
              <button
                ...
                className="self-end sm:self-center px-3.5 py-2 rounded-xl bg-[#061e14]/90 hover:bg-rose-950/70 text-zinc-300 hover:text-rose-200 border border-white/[0.1] hover:border-rose-500/50 text-xs font-semibold flex items-center gap-1.5 transition-all shadow-sm cursor-pointer"
          ...
          {room.lastDeflectAttempt && (
            <div className="mt-4 pt-4 border-t border-white/[0.08] flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
              <div className="text-zinc-400 flex items-center gap-2">
                ...
                <strong className="text-white font-mono bg-[#04160e] px-2.5 py-1 rounded-lg border border-white/[0.08] text-amber-300">
        ...
        <div className="glass-panel-elevated rounded-2xl p-3.5 border border-white/[0.07] specular-border flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-1.5 items-center">
            ...
            {room.askedWords?.map((w) => (
              <span
                key={`asked-${w}`}
                className="px-2.5 py-1 rounded-xl bg-[#04160e] text-emerald-200/80 border border-emerald-500/20 text-xs font-mono font-medium flex items-center gap-1 shadow-sm"
                title="Уже загадывалось (нельзя повторно загадывать или использовать как отгадку)"
              >
                <span className="text-[10px] text-emerald-400/60">Загадано:</span>
                <span className="line-through">{w.toUpperCase()}</span>
              </span>
            ))}
          </div>
        ...
        <div className="glass-panel-elevated rounded-3xl p-5 sm:p-7 border border-amber-400/40 shadow-[0_0_40px_rgba(245,158,11,0.25)] relative overflow-hidden specular-border">
          <div className="flex items-center justify-between flex-wrap gap-2 mb-2">
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-2xl bg-gradient-to-tr from-emerald-500/25 to-lime-500/25 border border-emerald-400/50 flex items-center justify-center text-emerald-300 shadow-[0_0_15px_rgba(16,185,129,0.35)]">
            ...
            {room.status === 'QUESTION_PHASE' && !hasActiveQuestion && room.activePlayerId && (
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-[#061e14] border border-emerald-400/40 text-xs font-mono text-emerald-300 shadow-[0_0_12px_rgba(16,185,129,0.2)]">
          ...
          <form onSubmit={handleDeflectSubmit} className="flex flex-col sm:flex-row gap-2.5">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-20 pr-4 py-3.5 rounded-2xl bg-[#061e14] text-amber-200 placeholder-zinc-700 border border-emerald-400/40 focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-400/40 font-bold font-mono text-base disabled:opacity-40 transition-all shadow-[0_0_15px_rgba(16,185,129,0.1)]"
              />
            </div>
            ...
            <button
              ...
              className="px-7 py-3.5 rounded-2xl bg-gradient-to-r from-emerald-400 via-lime-400 to-amber-300 hover:from-emerald-300 hover:to-amber-200 text-slate-950 font-black flex items-center justify-center gap-2 shadow-[0_0_35px_rgba(16,185,129,0.65)] border border-lime-300/80 disabled:opacity-40 transition-all cursor-pointer active:scale-95"
            >
              <Shield className="w-4 h-4" />
              <span>Отбить!</span>
            </button>
            ...
            <div>
              {hasJoinedContact ? (
                ...
                <button
                  ...
                  className="w-full py-4 px-6 rounded-2xl bg-gradient-to-r from-emerald-600/30 via-teal-600/30 to-lime-600/30 hover:from-emerald-500/45 hover:to-lime-500/45 text-emerald-200 hover:text-white font-bold text-base sm:text-lg shadow-[0_0_25px_rgba(16,185,129,0.3)] flex items-center justify-center gap-2.5 transition-all border border-emerald-400/40 hover:border-emerald-300 cursor-pointer active:scale-98"
                >
                  <Users className="w-5 h-5 text-emerald-300 drop-shadow-[0_0_6px_#34d399]" />
                  <span>Я тоже знаю! Поддержать контакт (+5 очков)</span>
                </button>
              ) : (
                /* Form for other players to enter their word */
                <div className="glass-panel-elevated rounded-3xl p-5 sm:p-6 border border-emerald-400/50 relative overflow-hidden specular-border animate-fade-in">
                  <div className="flex items-center justify-between mb-2">
                  ...
                  <p className="text-xs text-zinc-300 mb-3 leading-relaxed">
                    ...
                    Если кто-то ошибётся — буква не откроется! Слово на: <strong className="text-lime-300 font-mono">{revealedPrefix}...</strong>
                  </p>
                  ...
                  <form onSubmit={handleJoinContactSubmit} className="flex flex-col sm:flex-row gap-2.5">
                    <input
                      ...
                      placeholder={`Слово на ${revealedPrefix}...`}
                      className="flex-1 px-4 py-3.5 rounded-2xl bg-[#061e14] text-emerald-200 font-mono font-bold text-base uppercase border border-emerald-400/45 focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-400/50 placeholder-zinc-700 shadow-[0_0_15px_rgba(16,185,129,0.15)]"
                    />
                    <button
                      type="submit"
                      disabled={!joinWord.trim() || isSubmitting}
                      className="px-6 py-3.5 rounded-2xl bg-gradient-to-r from-emerald-500 via-lime-400 to-amber-300 hover:from-emerald-400 hover:to-amber-200 text-slate-950 font-black text-sm uppercase shadow-[0_0_25px_rgba(16,185,129,0.5)] border border-lime-300/60 flex items-center justify-center gap-2 disabled:opacity-50 transition-all cursor-pointer active:scale-95"
                    >
          ...
          {!hasActiveQuestion && (
            <div>
              {isMyTurn ? (
                ...
                <div className="glass-panel-elevated rounded-3xl p-5 sm:p-7 border border-emerald-400/50 shadow-[0_0_40px_rgba(16,185,129,0.3)] relative overflow-hidden specular-border animate-fade-in">
                  {/* 20s Turn Progress Bar */}
                  {room.turnExpiresAt && (
                    <div className="w-full bg-black/50 h-2 rounded-full overflow-hidden mb-4 border border-emerald-500/20 p-0.5">
                      <div
                        className={`h-full transition-all duration-150 rounded-full ${
                          turnSecondsLeft <= 3
                            ? 'bg-gradient-to-r from-rose-600 to-red-500 shadow-[0_0_18px_#f43f5e]'
                            : turnSecondsLeft <= 6
                            ? 'bg-gradient-to-r from-amber-500 to-yellow-400 shadow-[0_0_16px_#f59e0b]'
                            : 'bg-gradient-to-r from-emerald-400 via-teal-400 to-lime-400 shadow-[0_0_16px_#34d399]'
                        }`}
                  ...
                  <div className="flex items-center justify-between mb-3.5 flex-wrap gap-2">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-emerald-500/30 to-lime-500/30 text-emerald-300 border border-emerald-400/50 flex items-center justify-center shadow-[0_0_18px_rgba(16,185,129,0.45)]">
                        <Mic className="w-5 h-5 animate-pulse text-emerald-300" />
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {room.turnExpiresAt && (
                        <div
                          className={`px-3 py-1.5 rounded-xl border flex items-center gap-1.5 text-xs font-mono font-bold transition-all shadow-[0_0_15px_rgba(16,185,129,0.2)] ${
                            turnSecondsLeft <= 3
                              ...
                              : 'bg-emerald-950/60 text-emerald-200 border-emerald-400/50 shadow-[0_0_15px_rgba(16,185,129,0.35)]'
                          }`}
                          title="Оставшееся время на озвучивание намёка и ввод слова"
                        >
                          <Clock className={`w-3.5 h-3.5 ${turnSecondsLeft <= 3 ? 'text-rose-400 animate-spin' : 'text-emerald-300'}`} />
                      ...
                      {isLobbyHost && onPassTurnTo && otherEligiblePlayers.length > 0 && (
                        <div className="relative">
                          <select
                            ...
                            className="px-3 py-1.5 rounded-xl bg-[#061e14] text-emerald-200 border border-emerald-400/40 text-xs font-semibold cursor-pointer focus:outline-none hover:border-emerald-300 transition-colors shadow-[0_0_12px_rgba(16,185,129,0.15)]"
                          ...
                          >
                            ...
                            {otherEligiblePlayers.map((p) => (
                              <option key={p.id} value={p.id} className="bg-[#0a2f20] text-white">
                                {p.name}
                              </option>
                            ))}
                          </select>
                      ...
                      <button
                        type="button"
                        onClick={onSkipTurn}
                        className="px-3.5 py-1.5 rounded-xl bg-[#061e14] hover:bg-[#0c3121] text-zinc-300 hover:text-white border border-white/[0.12] hover:border-emerald-400/40 text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
                  ...
                  <div className="text-xs text-zinc-300 mb-4 leading-relaxed bg-[#061e14]/70 p-3.5 rounded-2xl border border-white/[0.08]">
                    <p className="mb-1">
                      <span className="inline-block w-4 h-4 rounded-full bg-emerald-500/25 text-emerald-300 text-[10px] font-bold text-center mr-1.5">1</span>
                      <strong>Озвучьте намёк голосом в Discord</strong> для всех участников.
                    </p>
                    <p>
                      <span className="inline-block w-4 h-4 rounded-full bg-emerald-500/25 text-emerald-300 text-[10px] font-bold text-center mr-1.5">2</span>
                      Введите сюда <strong>секретное слово-отгадку</strong> (начинается на <strong className="text-lime-300 font-mono">{revealedPrefix}...</strong>):
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleAskSubmit} className="flex flex-col gap-3">
                    <div>
                      <div className="relative">
                        <input
                          ...
                          className="w-full px-4 py-3.5 rounded-2xl bg-[#061e14] text-emerald-200 placeholder-emerald-800/60 border border-emerald-400/40 focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-400/50 font-bold uppercase tracking-wider text-base font-mono transition-all shadow-[0_0_15px_rgba(16,185,129,0.15)]"
                        />
                      </div>
                    ...
                    <div className="flex items-center justify-between gap-2 mt-1">
                      <button
                        type="button"
                        onClick={onSkipTurn}
                        className="px-4 py-3 rounded-xl bg-[#061e14] hover:bg-[#0c3121] text-zinc-400 hover:text-zinc-200 border border-white/[0.1] text-xs font-semibold transition-colors cursor-pointer"
                      >
                        Нет идей? Пропустить
                      </button>
                      ...
                      <button
                        ...
                        className="px-7 py-3 rounded-2xl bg-gradient-to-r from-emerald-400 via-teal-400 to-lime-400 hover:from-emerald-300 hover:to-lime-300 text-slate-950 font-black text-sm flex items-center justify-center gap-2 shadow-[0_0_30px_rgba(16,185,129,0.55)] border border-emerald-200/60 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed transition-all cursor-pointer"
                      >
                ...
                <div className="glass-panel-elevated rounded-3xl p-6 sm:p-8 text-center relative overflow-hidden specular-border border border-emerald-400/30 shadow-[0_0_35px_rgba(16,185,129,0.18)]">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-emerald-500/20 to-lime-500/20 text-emerald-300 border border-emerald-400/40 flex items-center justify-center mx-auto mb-3 shadow-[0_0_18px_rgba(16,185,129,0.3)]">
                  ...
                  <h4 className="text-base sm:text-lg font-black text-white mb-1 tracking-tight">
                    Очередь игрока: <span className="text-emerald-300 drop-shadow-[0_0_8px_rgba(52,211,153,0.6)]">{activePlayerName}</span>
                  </h4>
                  {room.turnExpiresAt && (
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-[#061e14] border border-emerald-400/30 text-xs font-mono text-emerald-200 my-2 shadow-[0_0_12px_rgba(16,185,129,0.15)]">
                      <Clock className={`w-3.5 h-3.5 ${turnSecondsLeft <= 3 ? 'text-rose-400 animate-pulse' : 'text-emerald-300'}`} />
                      <span>
                        Осталось времени на ход:{' '}
                        <strong className={turnSecondsLeft <= 3 ? 'text-rose-300 font-bold' : 'text-lime-300 font-bold'}>
                          {turnSecondsLeft} сек
                        </strong>
                      </span>
                    </div>
            ...
            <button
              ...
              className={`px-5 py-3 rounded-2xl text-xs sm:text-sm font-bold flex items-center gap-2.5 transition-all cursor-pointer ${
                remainingCooldownSec > 0
                  ...
                  : 'bg-gradient-to-r from-emerald-700/30 via-teal-600/30 to-lime-600/30 hover:from-emerald-600/45 hover:to-lime-600/45 text-emerald-200 hover:text-white border border-emerald-400/50 hover:border-emerald-300 shadow-[0_0_25px_rgba(16,185,129,0.35)] active:scale-95'
              }`}
            >
              <KeyRound className={`w-4 h-4 ${remainingCooldownSec > 0 ? 'text-amber-400' : 'text-emerald-400 drop-shadow-[0_0_6px_#34d399]'}`} />
              <span>
                {remainingCooldownSec > 0
